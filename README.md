...
轻量、自托管的 Markdown 阅读器。既可专注阅读单篇文档，也可选择原文和译文进行同步对照。
...
- 可收起的章节目录支持当前章节高亮和快速跳转
- 字号、行高和正文宽度可调，并保存在当前浏览器中
- 对照阅读可切换双栏、仅原文或仅译文；沉浸模式隐藏全部工具栏，桌面端按 `Esc` 退出，手机端轻点正文空白处可短暂显示退出按钮
...
1. 准备一篇 Markdown；如需对照阅读，再准备对应的译文。
2. 点击“临时打开文件”在当前页面中阅读，或点击“导入到书库”长期保存；也可以将现有文档放进 `documents/`。
3. 选择“单页阅读”或“对照阅读”，选好文档后开始阅读。
